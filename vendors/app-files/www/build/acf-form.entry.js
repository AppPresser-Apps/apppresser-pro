...
const AcfForm = class {
  ...
  validNote(input, parent, inputData) {
    ...
    if (valid.success !== false) {
      ...
      if (attrs[`${attrs.input_type}_required`]) {
        ...
        note.innerText = (_a = attrs[`${attrs.input_type}_validation_error_message`]) !== null && _a !== void 0 ? _a : `Invalid ${attrs.label}`;
      }
  ...
  validateInput(input, attr) {
    ...
    const email = mod.string().email();
    const url = mod.string().url();
  ...
  render() {
    return (h(Host, null, h("div", null, this.innerBlocks && this.innerBlocks.map(block => renderComponent(block)), this.data.attrs.data.hidden_inputs.map(item => (h("input", { type: "hidden", name: item.name, value: item.value }))))));
  }
  get el() { return getElement(this); }
};
AcfForm.style = acfFormCss;

export { AcfForm as acf_form };
