...
import { a as isPlatform, c as config, g as getIonMode } from './ionic-global-74a19eaa.js';
    ...
    if (Build.isBrowser) {
      rIC(async () => {
        ...
        if (!config.getBoolean('_testing')) {
          import('./index-d8e1c8ed.js').then((module) => module.startTapClick(config));
        }
        if (config.getBoolean('statusTap', isHybrid)) {
          import('./status-tap-15e5bf22.js').then((module) => module.startStatusTap());
        }
        if (config.getBoolean('inputShims', needInputShims())) {
          ...
          import('./input-shims-f96f4cd4.js').then((module) => module.startInputShims(config, platform));
        }
