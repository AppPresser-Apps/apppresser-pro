...
import { a as alertController, b as actionSheetController, m as modalController, p as popoverController } from './overlays-ef00d22b.js';
...
async function presentActionSheet(options) {
  const action_sheets = 'action_sheets' in state.data ? state.data['action_sheets'] : [];
  const blocks = action_sheets.filter(function (obj) {
      ...
      buttons.push({
        ...
        handler: () => {
          if ('custom' === item.action) {
            ...
            root.emitActionEvent(item.function, state);
          }
        }
      });
    });
...
async function presentPopover(options) {
...
function objectToUrlParams(params, prefix = '?') {
  return prefix + serializeToPhpQuery(params);
}
function serializeToPhpQuery(object, prefix = '') {
  let str = [], p;
  for (p in object) {
  ...
  switch (data.formatting) {
    ...
    case 'date':
      try {
        ...
        f_text = format(timestamp, (_a = data.date_format) !== null && _a !== void 0 ? _a : 'MMM do y p');
      }
      catch (error) {
...
export { presentActionSheet as a, presentPopover as b, presentAlert as c, dismissModal as d, formatText as f, objectToUrlParams as o, presentModal as p };
