...
function appp_acf_dynamic_icons( $field ) {
	...
	$icons = json_decode( $a, true );

	$field['choices'] = array( 0 => '(None)' );
	foreach ( $icons['icons'] as $icon => $value ) {
		$field['choices'][ $value['name'] ] = ucwords( str_replace( '-', ' ', $value['name'] ) );
	}

	return $field;
}
add_filter( 'acf/load_field/name=icon', 'appp_acf_dynamic_icons' );
